...
from .back_ends import ResNetBackEnd, Bottleneck, AlexNetBackEnd, CORnetSBackEnd,CORnetRBackEnd,CORnetR_AlexBackEnd,CORnetS_1
...
def VOneNet(sf_corr=0.75, sf_max=9, sf_min=0, rand_param=False, gabor_seed=0,
    ...
    if model_arch:
        ...
        elif model_arch.lower()=='cornetr':
            print('Model:','VOneCORnet-R')
            model_back_end=CORnetRBackEnd()
        elif model_arch.lower()=='cornetalex':
            print("Model:","CORnetR_Alex")
            model_back_end=CORnetR_AlexBackEnd()
        v_block=CORnetS_1()
