...
class CORnetRBackEnd(nn.Module):
    def __init__(self,times=5):
        super(CORnetRBackEnd, self).__init__()
        self.times = times
...
class CORnetR_AlexBackEnd(nn.Module):
    def __init__(self,times=5):
        super(CORnetR_AlexBackEnd, self).__init__()
        self.times = times
        self.V2 = CORblock_R(64, 192, stride=2, out_shape=28)
        self.features = nn.Sequential(
            nn.Conv2d(192, 384, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(384, 256, kernel_size=3, padding=1),
...
#alex-------------------------------------
class CORnetS_1(nn.Module):
    def __init__(self):
        super(CORnetS_1, self).__init__()
        self.bottleneck = nn.Conv2d(512, 64, kernel_size=1, stride=1, bias=False)
        self.V2 = CORblock_S(64, 128, times=2)
        self.V4 = CORblock_S(128, 256, times=4)
        self.IT = CORblock_S(256, 512, times=2)
