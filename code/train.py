...
def load_model():
    ...
    if FLAGS.ngpus > 0 and torch.cuda.device_count() > 1:
        ...
        model = model.module
#---------------------------------------------------------------------------------------------------------
#            load the vone-block parameters
#---------------------------------------------------------------------------------------------------------
...
def check_all_parameters(model):
    for name, param in model.named_parameters():
        if param.requires_grad:
            ...
            print(f"不可训练参数: {name}, 形状: {param.shape}")
...
#         self.model.eval()
#         start = time.time()
#         record = {'loss': 0, 'top1': 0, 'top5': 0, 'count': 0}  # Add count to accumulate the number of samples
#         with torch.no_grad():
#             for (inp, target) in tqdm.tqdm(self.data_loader, desc=self.name):
#                 target = target.to(device)
#                 output = self.model(inp)
...
#                 record['top1'] += p1
#                 record['top5'] += p5
#                 record['count'] += len(target)  # Accumulate the number of samples processed
